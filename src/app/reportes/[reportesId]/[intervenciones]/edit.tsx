...
export default function EditarIntervenciones(params: {id:string}) {
    
    const id = params.id;
    const {data: intervencion} = api.intervenciones.get.useQuery({id: id});
    ...
    const [createdAt, setCreatedAt] = useState<Date | undefined>( intervencion?.createdAt ?? new Date())
    const [userId, setUserId] = useState(intervencion?.usuario.id ?? "")
    const [title, setTitle] = useState(intervencion?.title ?? "");
    const [descripcion, setDescripcion] = useState(intervencion?.descripcion ?? "");
    const [ordenTrabajo, setOrdenTrabajo] = useState(intervencion?.ordenesTrabajo.id ?? "")
    // const [userId, setUserId] = useState( intervencion?.usuario?.id ?? "");
                        ...
                        <DialogContent className="bg-white shadow-lg rounded-lg p-6 sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
                            <DialogHeader className="border-b border-gray-200 pb-4 mb-4">
                                <DialogTitle className="text-xl font-semibold text-gray-800">
                                    {intervencion ? "Editar Orden de Trabajo" + " " + intervencion.title : "Crear Orden de Trabajo"}
                                </DialogTitle>
                            </DialogHeader>
